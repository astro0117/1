...
def test_copy_merge_functions():
    """测试复制和合并功能"""
    print("🧪 测试复制和合并功能")

    fs = EnhancedFileSystem(disk_size_mb=3)

    # 测试1: 创建测试文件
    fs.create_file("original.txt")
    fd = fs.open_file("original.txt", "w")
    fs.write_file(fd, b"Original content\nLine 1\nLine 2")
    fs.close_file(fd)

    print(" ✓ 创建测试文件")

    # 测试2: 复制文件
    assert fs.copy_file("original.txt", "copy.txt"), "复制文件失败"

    # 验证复制的内容
    fd = fs.open_file("copy.txt", "r")
    data = fs.read_file(fd, 100)
    fs.close_file(fd)
    assert data == b"Original content\nLine 1\nLine 2", f"复制内容不匹配: {data}"

    print(" ✓ 复制文件成功")

    # 测试3: 创建第二个文件
    fs.create_file("second.txt")
    fd = fs.open_file("second.txt", "w")
    fs.write_file(fd, b"Second file\nLine A\nLine B")
    fs.close_file(fd)

    print(" ✓ 创建第二个文件")

    # 测试4: 合并文件
    assert fs.merge_files("original.txt", "second.txt", "merged.txt"), "合并文件失败"

    # 验证合并的内容
    fd = fs.open_file("merged.txt", "r")
    merged_data = fs.read_file(fd, 200)
    fs.close_file(fd)

    # 检查是否包含两个文件的内容
    assert b"Original content" in merged_data, "第一个文件内容丢失"
    assert b"Second file" in merged_data, "第二个文件内容丢失"

    print(" ✓ 合并文件成功")

    # 测试5: 复制权限
    # 修改原始文件权限
    assert fs.change_permissions("original.txt", 0o600), "修改权限失败"

    # 复制并保持权限
    assert fs.copy_file("original.txt", "copy_with_perm.txt",
                        preserve_permissions=True), "带权限复制失败"

    info = fs.get_file_info("copy_with_perm.txt")
    assert info['permissions_octal'] == '0o600', f"权限未保持: {info['permissions_octal']}"

    print(" ✓ 复制保持权限")

    # 测试6: 复制不保持权限
    assert fs.copy_file("original.txt", "copy_no_perm.txt",
                        preserve_permissions=False), "不带权限复制失败"

    info2 = fs.get_file_info("copy_no_perm.txt")
    assert info2['permissions_octal'] != '0o600', f"权限应该不同: {info2['permissions_octal']}"

    print(" ✓ 复制不保持权限")

    # 测试7: 覆盖现有文件
    # 先创建目标文件
    fs.create_file("overwrite_target.txt")
    fd = fs.open_file("overwrite_target.txt", "w")
    fs.write_file(fd, b"Old content")
    fs.close_file(fd)

    # 复制并覆盖
    assert fs.copy_file("original.txt", "overwrite_target.txt"), "覆盖复制失败"

    fd = fs.open_file("overwrite_target.txt", "r")
    overwritten_data = fs.read_file(fd, 100)
    fs.close_file(fd)
    assert b"Original content" in overwritten_data, "覆盖后内容不正确"

    print(" ✓ 覆盖复制文件")

    # 测试8: 合并带分隔符
    assert fs.merge_files("original.txt", "second.txt", "merged_with_sep.txt",
                          separator=b"---\n"), "带分隔符合并失败"

    fd = fs.open_file("merged_with_sep.txt", "r")
    merged_with_sep = fs.read_file(fd, 200)
    fs.close_file(fd)
    assert b"---\n" in merged_with_sep, "分隔符未正确添加"

    print(" ✓ 带分隔符合并文件")

    # 清理
    for file in ["original.txt", "copy.txt", "second.txt", "merged.txt",
                 "copy_with_perm.txt", "copy_no_perm.txt",
                 "overwrite_target.txt", "merged_with_sep.txt"]:
        fs.delete(file)
...
def test_performance():
    ...
    for i in range(min(file_count, 5)):
        fs.copy_file(f"perf_test_{i}.txt", f"perf_test_copy_{i}.txt")
    copy_time = time.time() - start_time
    print(f" ✓ 复制 {min(file_count, 5)} 个文件: {copy_time:.2f}秒 ({min(file_count, 5) / copy_time:.1f} 文件/秒)")

    # 清理
    for i in range(file_count):
        fs.delete(f"perf_test_{i}.txt")
        if i < 5:
            fs.delete(f"perf_test_copy_{i}.txt")
...
def test_error_handling():
    ...
    # 创建测试用户
    fs.create_user("testuser", "testpass")

    # 以testuser登录
    fs.login("testuser", "testpass")

    # 创建文件
    assert fs.create_file("permission_test.txt"), "创建文件失败"
    ...
    # 修改权限为只读
    fs.change_permissions("permission_test.txt", 0o444)  # 只读
    ...
    # 复制不存在的文件
    result = fs.copy_file("nonexistent_src.txt", "dest.txt")
    assert not result, "应该无法复制不存在的文件"
    print(" ✓ 拒绝复制不存在的文件")

    # 合并不存在的文件
    result = fs.merge_files("nonexistent1.txt", "nonexistent2.txt", "output.txt")
    assert not result, "应该无法合并不存在的文件"
    print(" ✓ 拒绝合并不存在的文件")
    ...
    fs.change_permissions("readonly_dir", 0o555)  # 只读目录

    result = fs.copy_file("permission_test.txt", "readonly_dir/copy.txt")
    assert not result, "应该无法复制到只读目录"
    print(" ✓ 复制到只读目录被拒绝")

    # 清理
    fs.delete("permission_test.txt")
    fs.delete("readonly_dir")
...
def main():
    ...
    tests = [
        ...
        (test_copy_merge_functions, "复制合并功能"),  # 新增
